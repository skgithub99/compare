...
set @hasCustomerJobSiteRows		=	@@rowcount

--Get the hashbyte expression to use for the given table.
if (@hasCustomerJobSiteRows		>	0)
begin
	set @hashExprOut	=	''
	...
	set @sql = replace('select CJID,CJCMP,CJCUS#,CJJOB#,CJDLCD,CJLOC,CJRCDT,CJJLOC,CJCOM,CJNAME,CJADR1,CJADR2,CJCITY,CJST,CJZIP,CJAREA,CJPHON,CJTAXD,CJTERR,CJSREP,CJPO#,[hashByteExpr] as [Hash] from [sourceDB].[sourceSchema].[sourceTB] t with (nolock)', '[hashByteExpr]', @hashExprOut)
	...
	exec GEARBOXSQLPROD.Integration.gbw.getCustomerJobSite @sessionID = @sessionID
	set @compData = (select t.CompData from GEARBOXSQLPROD.Integration.gbw.CompData t where t.SessionID = @sessionID and t.Op = 'gbw.getCustomerJobSite')

	--Get response from  GB.
	if (@compData is not null and len(@compData) > 0)
	begin
	...
	begin
		...
		exec GEARBOXSQLPROD.Integration.gbw.upsertCustomerJobSite
			@sessionID = @sessionID
			,@comp = @compData
	end
end
...
--Get active PCs.
exec GEARBOXSQLPROD.Integration.gbw.getActivePCList @sessionID = @sessionID
set @compData = (select t.CompData from GEARBOXSQLPROD.Integration.gbw.CompData t where t.SessionID = @sessionID and t.Op = 'gbw.getActivePCList')
set @json = cast(decompress(@compData) as nvarchar(max))
...
--Get GB SKU list.
exec GEARBOXSQLPROD.Integration.gbw.getSKUList @sessionID = @sessionID
set @compData = (select t.CompData from GEARBOXSQLPROD.Integration.gbw.CompData t where t.SessionID = @sessionID and t.Op = 'gbw.getSKUList')
set @json = cast(decompress(@compData) as nvarchar(max))
...
--Get GB inventory.
exec GEARBOXSQLPROD.Integration.gbw.getInventory @sessionID = @sessionID
set @compData = (select t.CompData from GEARBOXSQLPROD.Integration.gbw.CompData t where t.SessionID = @sessionID and t.Op = 'gbw.getInventory')
set @json = cast(decompress(@compData) as nvarchar(max))
...
begin
	...
	--Add the SQL statement, using the hash statement that will populate the cache table.
	set @sql = replace('select t.EMCMP, t.[EMEQP#], [hashByteExpr] as [Hash] from [sourceDB].[sourceSchema].[sourceTB] t with (nolock)', '[hashByteExpr]', @hashExprOut)
	...
	select
		...
		,trim(em.EMCMP)
		,trim(em.[EMSER#])
	from WSDATA.WSDATA.EQPMASFL em with (nolock)
	join #tGBActivePC pc
		on pc.CMP		=	em.EMCMP
		and pc.PC		=	em.EMALOC
		and em.EMSTAT		=	'A'
...
begin
	...
	--Add the SQL statement, using the hash statement that will populate the cache table.
	set @sql = replace('select t.SYNCDTTM, t.RRN_FIELD_DATA, t.ELEVENT, t.ELCATG, t.ELCLAS, t.ELLOC, t.ELCMP, t.ELQOWN, [hashByteExpr] as [Hash] from [sourceDB].[sourceSchema].[sourceTB] t with (nolock)', '[hashByteExpr]', @hashExprOut)
	set @sql = replace(@sql, '[sourceDB]', @sourceDB)
	set @sql = replace(@sql, '[sourceTB]', @sourceTB)
	set @sql = replace(@sql, '[sourceSchema]', @sourceSchema)
	...
	(
		...
		from WSDATA.WSDATA.EQPCCLTRG t with (nolock)
		join
		(
			select
				t.ELCMP
...
begin try
	...
	from [sourceDB].[sourceSchema].[sourceTB] t with (nolock)
	...
	--This value is the GB PC # that we want to do a force refresh of serialized assets
	--that may need to be sent over to GB from Wynne.
	if (@gbAssetForceLoad > 0)
	begin
		set @sql = N'insert into #tEQPMASFL_Cache (EMCMP, [EMEQP#], ChangeDate, [Hash])
		select distinct
			t.EMCMP
		...
		join #tGBActivePC pc
			on pc.CMP = t.EMCMP
			and pc.PC = t.EMALOC
		left join #tEQPMASFL_Cache c
			on c.EMCMP = t.EMCMP
			and c.[EMEQP#] = t.[EMEQP#]
		where pc.PC = ''[gbAssetForceLoad]''
		and c.EMCMP is null
		and c.[EMEQP#] is null'
		...
		set @sql = replace(@sql, '[startDateTime]', convert(varchar(27), @startTime))
		set @sql = replace(@sql, '[endDateTime]', convert(varchar(27), @endTime))
		set @sql = replace(@sql, '[gbAssetForceLoad]', right('0000' + convert(varchar(4), @gbAssetForceLoad), 4))
		set @sql = replace(@sql, '[hashByteExpr]', @hashExprOut)

		--Store 'forced' changes.
		exec (@sql)
	...
	join [sourceDB].[sourceSchema].[sourceTB] e with (nolock)
	...
	(
		...
		from #tEQPCCLTRG_Cache tc
		join [sourceDB].[sourceSchema].EQPCCMFL ccm with (nolock)
	...
	join [sourceDB].[sourceSchema].[sourceTB] il with (nolock)
	...
	join [sourceDB].[sourceSchema].EQPMASFL e with (nolock)
		on chg.EMCMP		=	e.EMCMP
		and chg.[EMEQP#]	=	e.[EMEQP#]
		and e.EMDLCD		=	''A''									--Active equipment only.
	join [sourceDB].[sourceSchema].EQPCCMFL ec with (nolock)
		on e.EMCMP			=	ec.ECCMP
	...
	join [sourceDB].[sourceSchema].EQPCCLTRG il with (nolock)
	...
	begin
		set @compData = compress(@json)
		exec GEARBOXSQLPROD.Integration.gbw.upsertSKUList
			@sessionID = @sessionID
			,@comp = @compData
	end
	...
	(
		...
		join [sourceDB].[sourceSchema].EQPCCMFL ecm with (nolock)
	...
	begin
		...
		exec GEARBOXSQLPROD.Integration.gbw.upsertBulkList
			@sessionID = @sessionID
			,@comp = @compData
	end
	-------------------------* Add bulk items to remote *-------------------------
	...
	select
		...
		,[hashByteExpr]
		,''[cacheDate]''
	from [[sourceDB]].[[sourceSchema]].EQPCCLTRG src with (nolock)
	join
	(
		--rows not in cache.
		select
			...
			,cache.ELLOC
			,cache.ELCMP
		from [targetDB].[targetSchema].[targetTB] cache with (nolock)
	...
	set
		...
		,t.[Hash]	=	[hashByteExpr]
	from [targetDB].[targetSchema].[targetTB] t with (updlock, paglock)
	join [sourceDB].[sourceSchema].[sourceTB] src with (nolock)
	...
	from [targetDB].[targetSchema].[targetTB] t with (nolock)
	right join [sourceDB].[sourceSchema].[sourceTB] src with (nolock)
		on src.ELCATG	=	t.ELCATG
		and src.ELCLAS	=	t.ELCLAS
		and src.ELLOC	=	t.ELLOC
		and src.ELCMP	=	t.ELCMP
	...
	(
		...
		join [WSDATA].[WSDATA].EQPMASFL em with (nolock)
	...
	begin
		set @compData = compress(@json)
		exec GEARBOXSQLPROD.Integration.gbw.upsertEquipmentList
			@sessionID = @sessionID
			,@comp = @compData
	end
	-------------------------* Are there equipment changes that need to be sent to GB? *-------------------------
	...
	(
		select
			...
			,[tableAlias].[EMEQP#]
			,[hashByteExpr] as [Hash]
		from [sourceDB].[sourceSchema].[sourceTB] [tableAlias] with (nolock)
	...
	on a.EMCMP				=	t.EMCMP
		and a.[EMEQP#]		=	t.[EMEQP#]
	join [[sourceDB]].[[sourceSchema]].EQPMASFL src with (nolock)
